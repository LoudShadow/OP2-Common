...
Domain::SetupCommBuffers(Int_t edgeNodes)
{
  ...
  // Boundary nodesets
  if (m_colLoc == 0)
    m_symmX = (Real_t*) malloc(edgeNodes*edgeNodes*sizeof(Real_t));
    // m_symmX.resize(edgeNodes*edgeNodes);
  if (m_rowLoc == 0)
  m_symmY = (Real_t*) malloc(edgeNodes*edgeNodes*sizeof(Real_t));
    // m_symmY.resize(edgeNodes*edgeNodes);
  if (m_planeLoc == 0)
  m_symmZ = (Real_t*) malloc(edgeNodes*edgeNodes*sizeof(Real_t));
